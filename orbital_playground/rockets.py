...
ROCKETS = {
    ...
    # Theoretical / advanced concepts
    "Orion": Rocket(
        "Orion Spacecraft", m0_kg=4_000_000, mf_kg=400_000, isp_s=10_000
    ),  # Nuclear pulse propulsion
    "Antimatter rocket": Rocket(
        "Antimatter", m0_kg=1_000_000, mf_kg=200_000, isp_s=1_000_000
    ),  # Antimatter drive
}
...
def get_rocket(name: str):
    if name not in ROCKETS:
        raise ValueError(
            f"Unknown rocket '{name}'. Try one from the list: {list_rockets()}"
        )
    return ROCKETS[name]
