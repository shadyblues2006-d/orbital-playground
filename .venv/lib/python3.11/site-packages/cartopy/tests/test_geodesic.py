...
class TestGeodesic:
    def setup_class(self):
        ...
        self.start_pts = np.column_stack([self.data.start_lon, self.data.start_lat])
        self.end_pts = np.column_stack([self.data.end_lon, self.data.end_lat])
    ...
    def test_direct(self):
        geod_dir = self.geod.direct(self.start_pts, self.data.start_azi, self.data.dist)
    ...
    def test_direct_broadcast(self):
        ...
        geod_dir1 = self.geod.direct(
            self.start_pts[0], self.data.start_azi[0], repeat_dists
        )
        geod_dir2 = self.geod.direct(
            repeat_start_pts, self.data.start_azi[0], self.data.dist[0]
        )
...
def test_geometry_length_multilinestring():
    ...
    geom = sgeom.MultiLineString(
        [sgeom.LineString(np.array([lhr, jfk])), sgeom.LineString(np.array([tul, jfk]))]
    )
