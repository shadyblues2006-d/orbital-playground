...
class Test_interpolate_to_grid:
    ...
    def test_data_extent(self):
        ...
        expected_s_grid = np.array(
            [
                [np.nan, 2.0, 3.0, 2.0, np.nan],
    ...
    def test_multiple_fields(self):
        ...
        x_grid, y_grid, s_grid1, s_grid2, s_grid3 = vec_trans._interpolate_to_grid(
            5, 3, self.x, self.y, self.s, self.s, self.s
        )
...
class Test_vector_scalar_to_grid:
    ...
    def test_no_transform(self):
        ...
        expected_v_grid = np.array(
            [
                [np.nan, 0.8, 0.3, 0.8, np.nan],
                [np.nan, 2.675, 2.15, 2.675, np.nan],
                [5.5, 4.75, 4.0, 4.75, 5.5],
            ]
        )
    ...
    def test_with_transform(self):
        ...
        input_coords = [
            src_crs.transform_point(xp, yp, target_crs)
            for xp, yp in zip(self.x, self.y)
        ]
        x_nps = np.array([ic[0] for ic in input_coords])
        y_nps = np.array([ic[1] for ic in input_coords])
        u_nps, v_nps = src_crs.transform_vectors(
            target_crs, self.x, self.y, self.u, self.v
        )
    ...
    def test_with_scalar_field(self):
        ...
        src_crs = target_crs = ccrs.PlateCarree()
        x_grid, y_grid, u_grid, v_grid, s_grid = vec_trans.vector_scalar_to_grid(
            src_crs, target_crs, (5, 3), self.x, self.y, self.u, self.v, self.s
        )
