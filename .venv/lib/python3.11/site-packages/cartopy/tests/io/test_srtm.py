...
def srtm_login_or_skip(monkeypatch):
    ...
    password_manager.add_password(
        None, "https://urs.earthdata.nasa.gov", srtm_username, srtm_password
    )
    cookie_jar = CookieJar()
    opener = build_opener(
        HTTPBasicAuthHandler(password_manager), HTTPCookieProcessor(cookie_jar)
    )
...
class TestRetrieve:
    @pytest.mark.parametrize(
        "Source, read_SRTM, max_, min_, pt",
        [
            (cartopy.io.srtm.SRTM3Source, cartopy.io.srtm.read_SRTM3, 602, -34, 78),
            (cartopy.io.srtm.SRTM1Source, cartopy.io.srtm.read_SRTM1, 602, -37, 50),
        ],
        ids=[
            "srtm3",
            "srtm1",
    ...
    def test_srtm_retrieve(
        self, Source, read_SRTM, max_, min_, pt, download_to_temp
    ):  # noqa: F811
