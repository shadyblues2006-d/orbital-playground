...
class TestRegrid:
    def test_array_dims(self):
        ...
        data = np.arange(source_nx * source_ny, dtype=np.int32).reshape(
            source_ny, source_nx
        )
        ...
        # Perform regrid
        new_array = im_trans.regrid(
            data, source_x, source_y, source_cs, target_proj, target_x, target_y
        )
    ...
    def test_different_dims(self):
        ...
        target_x = np.arange(
            reduce(operator.mul, target_x_shape), dtype=np.float64
        ).reshape(target_x_shape)
        target_y = np.arange(
            reduce(operator.mul, target_y_shape), dtype=np.float64
...
def test_regrid_image():
    ...
    # Perform regrid
    new_array = im_trans.regrid(
        data, source_x, source_y, source_proj, target_proj, target_x, target_y
    )
    ...
    for i, color in enumerate(["red", "green", "blue"]):
        ...
        ax.imshow(
            new_array[:, :, i], extent=target_extent, origin="lower", cmap=cmaps[color]
        )
        ax.coastlines()
