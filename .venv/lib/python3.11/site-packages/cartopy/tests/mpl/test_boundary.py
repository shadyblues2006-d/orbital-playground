...
circle_codes = [Path.MOVETO, *[Path.LINETO] * (len(circle_verts)), Path.CLOSEPOLY]
...
rectangle_codes = [Path.MOVETO, *[Path.LINETO] * (len(rectangle_verts)), Path.CLOSEPOLY]
...
def test_multi_path_boundary():
    ...
    # add rectangle
    vertices.extend([rectangle_verts[0], *rectangle_verts, rectangle_verts[-1]])
    codes.extend([Path.MOVETO, *[Path.LINETO] * len(rectangle_verts), Path.CLOSEPOLY])
