...
def test_default():
    ...
    assert_allclose(np.array(moll.x_limits), [-18040095.6961473, 18040095.6961473])
    assert_allclose(np.array(moll.y_limits), [-9020047.8480736, 9020047.8480736])
...
def test_grid():
    ...
    globe = ccrs.Globe(ellipse=None, semimajor_axis=0.5**0.5, semiminor_axis=0.5**0.5)
    moll = ccrs.Mollweide(globe=globe)
...
def test_sphere_transform():
    ...
    moll = ccrs.Mollweide(central_longitude=-90.0, globe=globe)
    geodetic = moll.as_geodetic()

    other_args = {"a=1.0", "b=1.0", "lon_0=-90.0"}
    check_proj_params("moll", moll, other_args)

    assert_allclose(np.array(moll.x_limits), [-2.8284271247461903, 2.8284271247461903])
    assert_allclose(np.array(moll.y_limits), [-1.4142135623730951, 1.4142135623730951])
