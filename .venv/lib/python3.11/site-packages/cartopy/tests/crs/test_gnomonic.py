...
def test_grid():
    ...
    expected_x = np.tile(
        [0.0000, 0.1763, 0.3640, 0.5774, 0.8391, 1.1918, 1.7321, 2.7475, 5.6713], 9
    )
    expected_y = np.array(
        [
    ...
    for lon_sign in [1, -1]:
        for lat_sign in [1, -1]:
            result = gnom.transform_points(geodetic, lon_sign * lons, lat_sign * lats)
...
def test_sphere_transform():
    ...
    gnom = ccrs.Gnomonic(central_latitude=40.0, central_longitude=-100.0, globe=globe)
    geodetic = gnom.as_geodetic()
