...
def test_central_params(lon, lat):
    ...
    other_args = {f"lat_0={lat}", f"lon_0={lon}", "a=6378137.0", "alpha=0.0"}
    check_proj_params("ortho", ortho, other_args)
...
def test_grid():
    ...
    for lon_sign in [1, -1]:
        for lat_sign in [1, -1]:
            result = ortho.transform_points(geodetic, lon_sign * lons, lat_sign * lats)
...
def test_sphere_rotate():
    globe = ccrs.Globe(semimajor_axis=1.0, semiminor_axis=1.0, ellipse=None)
    ortho = ccrs.Orthographic(
        central_latitude=40.0, central_longitude=-100.0, azimuth=180.0, globe=globe
    )
    geodetic = ortho.as_geodetic()
