...
class TestAzimuthalEquidistant:
    def test_default(self):
        ...
        other_args = {"ellps=WGS84", "lon_0=0.0", "lat_0=0.0", "x_0=0.0", "y_0=0.0"}
        check_proj_params("aeqd", aeqd, other_args)

        assert_almost_equal(
            np.array(aeqd.x_limits), [-20037508.34278924, 20037508.34278924], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-19970326.371123, 19970326.371123], decimal=6
        )

    def test_eccentric_globe(self):
        globe = ccrs.Globe(semimajor_axis=1000, semiminor_axis=500, ellipse=None)
        aeqd = ccrs.AzimuthalEquidistant(globe=globe)
        other_args = {"a=1000", "b=500", "lon_0=0.0", "lat_0=0.0", "x_0=0.0", "y_0=0.0"}
        check_proj_params("aeqd", aeqd, other_args)

        assert_almost_equal(
            np.array(aeqd.x_limits), [-3141.59265359, 3141.59265359], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-1570.796326795, 1570.796326795], decimal=6
        )

    def test_eastings(self):
        aeqd_offset = ccrs.AzimuthalEquidistant(
            false_easting=1234, false_northing=-4321
        )
        ...
        assert_almost_equal(
            np.array(aeqd_offset.x_limits),
            [-20036274.34278924, 20038742.34278924],
            decimal=6,
        )
        assert_almost_equal(
            np.array(aeqd_offset.y_limits),
            [-19974647.371123, 19966005.371123],
            decimal=6,
        )
    ...
    def test_grid(self):
        ...
        aeqd = ccrs.AzimuthalEquidistant(
            central_latitude=0.0, central_longitude=0.0, globe=globe
        )
        geodetic = aeqd.as_geodetic()

        other_args = {"a=1.0", "b=1.0", "lon_0=0.0", "lat_0=0.0", "x_0=0.0", "y_0=0.0"}
        check_proj_params("aeqd", aeqd, other_args)

        assert_almost_equal(
            np.array(aeqd.x_limits), [-3.14159265, 3.14159265], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-3.14159265, 3.14159265], decimal=6
        )
    ...
    def test_sphere_transform(self):
        ...
        globe = ccrs.Globe(ellipse=None, semimajor_axis=3.0, semiminor_axis=3.0)
        aeqd = ccrs.AzimuthalEquidistant(
            central_latitude=40.0, central_longitude=-100.0, globe=globe
        )
        geodetic = aeqd.as_geodetic()
        ...
        assert_almost_equal(
            np.array(aeqd.x_limits), [-9.42477796, 9.42477796], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-9.42477796, 9.42477796], decimal=6
        )
    ...
    def test_ellipsoid_polar_transform(self):
        ...
        aeqd = ccrs.AzimuthalEquidistant(
            central_latitude=90.0, central_longitude=-100.0, globe=globe
        )
        geodetic = aeqd.as_geodetic()

        other_args = {
            "a=6378388.0",
            "f=0.003367003355798981",
            "lon_0=-100.0",
            "lat_0=90.0",
            "x_0=0.0",
            "y_0=0.0",
        ...
        assert_almost_equal(
            np.array(aeqd.x_limits), [-20038296.88254529, 20038296.88254529], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-19970827.86969727, 19970827.86969727], decimal=6
        )
    ...
    def test_ellipsoid_guam_transform(self):
        ...
        assert_almost_equal(
            np.array(aeqd.x_limits), [-19987726.36931940, 20087726.36931940], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-19919796.94787477, 20019796.94787477], decimal=6
        )
    ...
    def test_ellipsoid_micronesia_transform(self):
        ...
        other_args = {
            "a=6378206.4",
            "f=0.003390076308689371",
            "lon_0=145.7416588888889",
            "lat_0=15.18491194444444",
            "x_0=28657.52",
            "y_0=67199.99000000001",
        }
        check_proj_params("aeqd", aeqd, other_args)

        assert_almost_equal(
            np.array(aeqd.x_limits), [-20009068.84931940, 20066383.88931940], decimal=6
        )
        assert_almost_equal(
            np.array(aeqd.y_limits), [-19902596.95787477, 20036996.93787477], decimal=6
        )
