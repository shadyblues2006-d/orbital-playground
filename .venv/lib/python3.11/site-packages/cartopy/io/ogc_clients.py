...
class WMTSRasterSource(RasterSource):
    ...
    def fetch_raster(self, projection, extent, target_resolution):
        ...
        if crs_urn in _URN_TO_CRS:
            ...
            raise ValueError(
                f"Unknown coordinate reference system string:" f" {crs_urn}"
            )
    ...
    def _wmts_images(self, wmts, layer, matrix_set_name, extent, max_pixel_span):
        ...
        if tile_matrix_set.crs in METERS_PER_UNIT:
            ...
            with warnings.catch_warnings():
                ...
                if crs_units != "m":
                    raise ValueError(
                        'Only unit "m" implemented for' " EPSG projections."
                    )
                meters_per_unit = 1
        else:
            raise ValueError(
                f"Unknown coordinate reference system string:" f" {tile_matrix_set.crs}"
            )
...
class WFSGeometrySource:
    ...
    def default_projection(self):
        ...
        if str(self._default_urn) in _URN_TO_CRS:
            ...
            raise ValueError(
                f"Unknown coordinate reference system:" f" {str(self._default_urn)}"
            )
