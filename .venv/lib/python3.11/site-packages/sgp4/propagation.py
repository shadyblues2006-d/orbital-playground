...
def sgp4(satrec, tsince, whichconst=None):
    ...
    if nm <= 0.0:
        ...
        satrec.error_message = "mean motion {0:f} is less than zero".format(nm)
        satrec.error = 2
        #  sgp4fix add return
        return false, false
    ...
    if em >= 1.0 or em < -0.001:  # || (am < 0.95)
        ...
        satrec.error_message = (
            "mean eccentricity {0:f} not within" " range 0.0 <= e < 1.0".format(em)
        )
        satrec.error = 1
    ...
    if satrec.method == "d":
        ...
        ep, xincp, nodep, argpp, mp = _dpper(
            satrec, satrec.inclo, "n", ep, xincp, nodep, argpp, mp, satrec.operationmode
        )
        if xincp < 0.0:
        ...
        if ep < 0.0 or ep > 1.0:
            ...
            satrec.error_message = (
                "perturbed eccentricity {0:f} not within"
    ...
    if pl < 0.0:
        ...
        satrec.error_message = "semilatus rectum {0:f} is less than zero".format(pl)
        satrec.error = 4
        #  sgp4fix add return
        return false, false
    ...
    if mrt < 1.0:
        ...
        satrec.error_message = (
            "mrt {0:f} is less than 1.0 indicating"
