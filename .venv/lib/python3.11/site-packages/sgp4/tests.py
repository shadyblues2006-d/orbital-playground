...
def test_check_satrec():
    ...
    msg = (
        "satellite parameters out of range:\n"
...
def test_satnum_alpha5_encoding():
    def make_sat(satnum_string):
        return Satrec.twoline2rv(
            LINE1.replace("00005", satnum_string), LINE2.replace("00005", satnum_string)
        )
...
def test_satnum_that_is_too_large():
    ...
    with assertRaisesRegex(ValueError, "cannot exceed 339999"):
        sat.sgp4init(WGS72, "i", 340000, VANGUARD_EPOCH, *sgp4init_args(VANGUARD_ATTRS))
...
def test_intldesg_with_7_characters():
    sat = Satrec.twoline2rv(
        "1 39444U 13066AE  20110.89708219  .00000236  00000-0" "  35029-4 0  9992",
        "2 39444  97.5597 114.3769 0059573 102.0933 258.6965 " "14.82098949344697",
    )
...
def test_legacy_against_tcppver():
    ...
    errs = [
        ...
        (6, "mrt 0.996159 is less than 1.0" " indicating the satellite has decayed"),
        (6, "mrt 0.996252 is less than 1.0" " indicating the satellite has decayed"),
        (4, "semilatus rectum -0.103223 is less than zero"),
        (3, "perturbed eccentricity -122.217193" " not within range 0.0 <= e <= 1.0"),
        (6, "mrt 0.830534 is less than 1.0" " indicating the satellite has decayed"),
    ]
...
def run_satellite_against_tcppver(twoline2rv, invoke, expected_errors):
    ...
    for lineno, (expected_line, actual_line) in enumerate(linepairs, start=1):
        ...
        if actual_line == "(Use previous data line)":
            actual_line = "       0.00000000" + previous_data_line[17:107]
        ...
        if "xx" in actual_line:
            ...
            similar = (
                ...
                and all(-error < (a - e) < error for a, e in zip(actual7, expected7))
                and afields[7:] == efields[7:]  # just compare text
            )
        ...
        if not similar:
            raise ValueError(
                ...
                "\n"
                "Expected: %r\n"
                "Got back: %r" % (lineno, expected_line, actual_line)
            )
...
def generate_test_output(twoline2rv, invoke, error_list):
    ...
    for line1 in tle_lines:
        ...
        for line in generate_satellite_output(satrec, invoke, line2, error_list):
            yield line
